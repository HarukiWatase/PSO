...
def simulation(Graph, gen, par):
    ...
    # シミュレーション関数の出力
    return gBests_bns[num_gen - 1], bn_inv
...
if __name__ == '__main__':
    ...
    for iter in range(num_simulation):
        ...
        # シミュレーション結果を一旦出力
        result_temp, bn_inv = simulation(Graph=Graph, gen=num_gen, par=num_par)
        # シミュレーション結果を格納
        bottleneck_links.append([iter, result_temp, bn_inv])
    
    # ファイル名を生成
    file_name = datetime.now().strftime('%Y%m%d_%H%M%S') + '_rich.csv'
    file_path = result_path + '/' + file_name
    
    # ファイルに書き出し
    with open(file_path, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['iter', 'Bottleneck', 'inverse'])
