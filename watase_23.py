# watase_20_pso_adaptive_termination_FIXED.py
...
def simulation(Graph, src, dst, constraints, pso_params):
    ...
    final_gen = num_gen # デフォルトは最大世代数

    for i in range(num_gen):
        current_pso_time = time.time() - pso_start_time
        
        # ★★★ 修正点: タイムアウト判定を最優先し、実行可能解の有無と無関係にする ★★★
...
if __name__ == '__main__':
    ...
    for num_nodes in node_counts:
        for graph_type in graph_types:
            for i in range(num_simulation):
                ...
                if len(node_list) < 2: continue # Ensure at least 2 nodes
    ...
    # --- CSVファイルへの書き出し ---
    # ★★★ 修正点: 新しいディレクトリとファイル名に出力 ★★★
    result_path = savef.create_dir(dir_name="adaptive_termination_results_4D_FIXED")
    filename = result_path + f'/{datetime.now().strftime("%Y%m%d_%H%M%S")}_4criteria_adaptive_FIXED.csv'
