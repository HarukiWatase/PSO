...
def simulation(Graph, gen, par, src_node, dst_node, delay_multiplier=1.5):
    ...
    num_pareto_paths = 0 # ★追加：パレート解の数を初期化
...
if __name__ == '__main__':
    ...
    node_counts = [1900,2100,2300] 

    # 固定パラメータ
    num_simulation = 50
    num_par = 300
    num_gen = 500
    delay_multipliers = [1.5,2.0,2.5,3.0]
    graph_types = ['ba']
    ...
    for num_nodes in node_counts:
        # 毎回の結果を一時的に保存するパスを生成（ノード数ごとにフォルダ分けも可能）
        result_path = savef.create_dir(dir_name=f"results_{num_nodes}nodes")
