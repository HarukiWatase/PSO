...
"""
[変更点 (watase_27)]
- 「経路修復 (Path Repair)」アルゴリズムの導入
- PathEncode が失敗 (is_valid=False) した場合、
  Fitness = -1.0 の罰を与える代わりに、
  Dijkstra法 (最短遅延パス) で修復し、そのパスのFitnessを
  評価値として返すように変更。
- これにより、局所解の罠からの脱出を狙う。
"""

# (PathEncode, calculate_path_attributes_4d, find_optimal_path_by_label_correcting_4d は watase_26_gen_log.py と同一)
...
def simulation(Graph, src, dst, constraints, pso_params):
    ...
    # ★★★ 変更点: 修復用のパスをあらかじめ計算 ★★★
    # (min_delay_path_dijkstra は遅延最小パスなので、これを修復用パスとして使う)
    repaired_path = min_delay_path_dijkstra
    ...
    for i in range(num_gen): # 世代ループ
        ...
        for j in range(num_par): # 粒子ループ
            ...
            # ▼▼▼ ★★★ 変更点: 経路修復ロジック ★★★ ▼▼▼
            if not is_valid:
                # 罰 (-1.0) を与える代わりに、修復パスで置き換える
                path = repaired_path
                # is_valid=False だったことは、この後のFitness計算には影響しない
            
            # (is_valid=True であっても、is_valid=False で修復された場合でも、
            #  必ず以下のFitness計算が実行される)
            ...
            # (gBest_feasible_bn の更新ロジックは watase_26_gen_log.py と同一)
...
if __name__ == '__main__':
    ...
    num_simulation = 30 # ★ 30回試行 (または 200回)
    node_counts = [1000]  # ★ 1000ノード
    graph_types = ['random']
    ...
    # ★★★ ファイル名に 'repair' を追加 ★★★
    generation_log_filename = result_path + f'/{run_timestamp}_4criteria_generation_log_REPAIR.csv'
    summary_filename = result_path + f'/{run_timestamp}_4criteria_summary_REPAIR.csv'
