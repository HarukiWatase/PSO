...
if __name__ == '__main__':
    num_simulation = 10   # 各条件下での試行回数
    num_nodes = 3000       # ノード数（大規模テストの場合は調整）
    num_par = 200         # 粒子数
    num_gen = 400         # 世代数
    ...
    for graph_type in graph_types:
        ...
        for multiplier in delay_multipliers:
            ...
            for iter_num in range(num_simulation):
                ...
                # ★変更点★ graph_typeに応じて生成するグラフを変更
                # (事前にcreate_graph.pyに各関数が定義されている必要があります)
                ...
                # ★変更点★ 結果リストにグラフタイプを追加
                results.append([
                    iter_num + 1, graph_type, multiplier, pso_bn, inv_dijkstra_bn,
                    mod_dijkstra_bn, label_correcting_bn, pso_delay,
                    pso_time, lc_time
                ])

    file_name = datetime.now().strftime('%Y%m%d_%H%M%S') + '_multi_topology.csv'
    file_path = result_path + '/' + file_name

    with open(file_path, 'w', newline='') as f:
        writer = csv.writer(f)
        # ★変更点★ CSVヘッダーにグラフタイプの列を追加
        writer.writerow([
            'iter', 'graph_type', 'delay_multiplier', 'PSO_Bottleneck', 'Inverse_Dijkstra_BN',
