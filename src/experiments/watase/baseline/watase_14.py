...
# ★★★ 変更点：グリッドサーチのため、各種パラメータ設定を引数で受け取る ★★★
def simulation(Graph, gen, par, src_node, dst_node, delay_multiplier=1.5,
               w_config=(0.9, 0.4), c1_config=(2.5, 0.5), c2_config=(0.5, 2.5),
               penalty_config=(0.1, 1.0)):
    num_node = int(len(Graph.nodes()))
    num_par = par
    num_gen = gen
    lb, ub = 1, 20

    w_start, w_end = w_config
    c1_start, c1_end = c1_config
    c2_start, c2_end = c2_config
    penalty_coeff_start, penalty_coeff_end = penalty_config
...
if __name__ == '__main__':
    # ==================================================================
    # ★★★ グリッドサーチのためのパラメータセットをここで定義 ★★★
    # ==================================================================
    
    # --- 調査軸1：探索コストの配分 (総評価回数 = 20,000) ---
    axis1_cost = {
        'Cost_A_Few_Elite': {'num_par': 50, 'num_gen': 400},
        'Cost_B_Balanced':  {'num_par': 100, 'num_gen': 200},
        'Cost_C_Swarm':     {'num_par': 200, 'num_gen': 100},
    }

    # --- 調査軸2：探索挙動の制御 ---
    axis2_behavior = {
        'Behav_A_Adaptive': {'w': (0.9, 0.4), 'c1': (2.5, 0.5), 'c2': (0.5, 2.5)},
        'Behav_B_Standard': {'w': (0.9, 0.4), 'c1': (2.0, 2.0), 'c2': (2.0, 2.0)},
        'Behav_C_Social':   {'w': (0.9, 0.4), 'c1': (1.5, 1.5), 'c2': (2.5, 2.5)},
    }

    # --- 調査軸3：制約処理の強度 ---
    axis3_penalty = {
        'Pen_A_Gentle':    {'penalty': (0.1, 1.0)},
        'Pen_B_Standard':  {'penalty': (0.5, 5.0)},
        'Pen_C_Aggressive':{'penalty': (1.0, 10.0)},
    }

    # ==================================================================
    # ★★★ 実験設定 ★★★
    # ==================================================================
    # --- 調査したいパラメータセットを選択 (1つだけコメントを外す) ---
    #param_sets_to_run = axis1_cost
    #param_sets_to_run = axis2_behavior
    param_sets_to_run = axis3_penalty

    # --- 固定するベースラインパラメータ ---
    baseline_cost = axis1_cost['Cost_B_Balanced']
    baseline_behavior = axis2_behavior['Behav_A_Adaptive']
    baseline_penalty = axis3_penalty['Pen_A_Gentle']

    # --- その他の実験設定 ---
    num_simulation = 50
    node_counts = [200,500] # ノード数固定
    delay_multipliers = [1.5,2.5]
    graph_types = ['random', 'ba']

    results = []

    # --- 実験ループ ---
    for param_id, current_params in param_sets_to_run.items():
        # ベースライン設定と現在の設定をマージ
        sim_params = {**baseline_cost, **baseline_behavior, **baseline_penalty, **current_params}
        
        print(f"\n############################################################")
        print(f"##### パラメータセット: {param_id} での実験を開始 #####")
        print(f"##### {sim_params} #####")
        print(f"############################################################")
            ...
            for graph_type in graph_types:
                for multiplier in delay_multipliers:
                    for iter_num in range(num_simulation):
                        print(f'\n--- 試行: {iter_num+1}/{num_simulation} (ID:{param_id}, Node:{num_nodes}, Graph:{graph_type}, Mul:{multiplier}) ---')
                        
                        if graph_type == 'random':
                            Graph = create_graph.rnd_graph(num_node=num_nodes)
                        elif graph_type == 'grid':
                            Graph = create_graph.create_grid_graph(num_node=num_nodes)
                        ...
                        pso_bn, inv_dijkstra_bn, pso_delay, mod_dijkstra_bn, label_correcting_bn, num_pareto, pso_time, lc_time = simulation(
                            Graph=Graph,
                            gen=sim_params['num_gen'],
                            par=sim_params['num_par'],
                            src_node=src_node,
                            dst_node=dst_node,
                            delay_multiplier=multiplier,
                            w_config=sim_params['w'],
                            c1_config=sim_params['c1'],
                            c2_config=sim_params['c2'],
                            penalty_config=sim_params['penalty']
                        )
                        
                        results.append([
                            param_id, num_nodes, iter_num + 1, graph_type, multiplier,
                            pso_bn, inv_dijkstra_bn, mod_dijkstra_bn, label_correcting_bn,
                            num_pareto, pso_delay, pso_time, lc_time
                        ])

    # --- CSVファイルへの書き出し ---
    file_name = datetime.now().strftime('%Y%m%d_%H%M%S') + '_grid_search_experiment.csv'
    result_path = savef.create_dir(dir_name="grid_search_results")
    file_path = result_path + '/' + file_name

    with open(file_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            'Param_ID', 'Num_Nodes', 'iter', 'graph_type', 'delay_multiplier',
            'PSO_Bottleneck', 'Inverse_Dijkstra_BN', 'Unconstrained_Mod_Dijkstra_BN',
            'Optimal_Bottleneck', 'Num_Pareto_Paths', 'PSO_Delay',
            'PSO_Time_sec', 'Optimal_Time_sec'
        ])
        writer.writerows(results)

    print(f"\nグリッドサーチ実験の結果が '{file_path}' に保存されました。")