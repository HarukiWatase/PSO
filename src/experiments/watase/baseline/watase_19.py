...
"""
This script logs PSO's convergence process generation by generation.
It outputs two separate CSV files: one for the summary and one for convergence details.
"""
...
def simulation(Graph, src, dst, constraints, pso_params):
    ...
    opt_path, opt_bn, _, _, _, num_pareto = find_optimal_path_by_label_correcting_4d(
    ...
    # ★★★ 変更点1: 収束過程を記録する変数を初期化 ★★★
    convergence_log = []
    first_feasible_gen = -1
    ...
    for i in range(num_gen):
        ...
        for j in range(num_par):
            ...
            if is_feasible:
                # ★★★ 変更点2: 初めて実行可能解が見つかった世代を記録 ★★★
                if first_feasible_gen == -1:
                    first_feasible_gen = i + 1
                if bn > gBest_feasible_bn:
        ...
        convergence_log.append({
            'generation': i + 1,
            'best_fitness_ever': gBest_fitness,
            'best_feasible_bottleneck': gBest_feasible_bn
        })
    ...
    # ★★★ 変更点4: 新しいデータを返す ★★★
    return (opt_bn, lc_time, num_pareto,
            pso_bn, pso_time, gBest_feasible_path is not None, 
            pso_d, pso_l, pso_r,
            first_feasible_gen, convergence_log)

if __name__ == '__main__':
    # ★★★ 変更点5: 2種類のCSVに結果を保存する ★★★
    summary_results = []
    convergence_results = []
    simulation_id_counter = 0
    ...
    pso_params = {
        'num_par': 150, 'num_gen': 50, 'w_config': (0.9, 0.4), 'c1_config': (2.5, 0.5), 'c2_config': (0.5, 2.5),
    ...
    for num_nodes in node_counts:
        for graph_type in graph_types:
            for i in range(num_simulation):
                ...
                print(f'\n--- Sim ID: {simulation_id_counter} (N:{num_nodes}, G:{graph_type}) ---')
                ...
                (opt_bn, lc_time, num_pareto,
                 pso_bn, pso_time, pso_feasible, 
                 pso_d, pso_l, pso_r,
                 first_gen, conv_log) = simulation(Graph, src, dst, constraints, pso_params)

                summary_results.append([
                    simulation_id_counter, num_nodes, graph_type, opt_bn, lc_time,
                    pso_bn, pso_time, pso_feasible, first_gen
                ])
                for log_entry in conv_log:
                    convergence_results.append([
                        simulation_id_counter, log_entry['generation'],
                        log_entry['best_fitness_ever'], log_entry['best_feasible_bottleneck']
                    ])

    # --- 2つのCSVファイルに書き出し ---
    result_path = savef.create_dir(dir_name="convergence_results")
    
    # サマリーファイル
    summary_filename = result_path + f'/{datetime.now().strftime("%Y%m%d_%H%M%S")}_summary.csv'
    with open(summary_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Sim_ID', 'Num_Nodes', 'Graph_Type', 'Optimal_BN', 'Optimal_Time',
                         'PSO_BN', 'PSO_Time', 'PSO_Feasible', 'First_Feasible_Gen'])
        writer.writerows(summary_results)
    print(f"\nサマリー結果が '{summary_filename}' に保存されました。")

    # 収束過程ファイル
    conv_filename = result_path + f'/{datetime.now().strftime("%Y%m%d_%H%M%S")}_convergence.csv'
    with open(conv_filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Sim_ID', 'Generation', 'Best_Fitness_Ever', 'Best_Feasible_BN'])
        writer.writerows(convergence_results)
    print(f"収束過程データが '{conv_filename}' に保存されました。")