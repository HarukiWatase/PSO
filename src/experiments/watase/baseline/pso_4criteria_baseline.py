"""Baseline PSO experiment for 4-criteria constrained routing.

This file is the canonical baseline implementation migrated from `watase_24.py`.
It compares label-correcting exact search with PSO and records progress updates
when feasible global-best bottleneck is improved.
"""
...
def simulation(Graph, src, dst, constraints, pso_params):
    """Run exact solver + PSO baseline and return metrics/logs for one graph."""
    # (厳密解法の実行部分は変更なし)
    try: 
