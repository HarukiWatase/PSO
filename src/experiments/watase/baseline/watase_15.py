...
if __name__ == '__main__':
    ...
    # ★★★ 変更点：調査軸2の組み合わせを詳細化 ★★★
    axis2_behavior = {
        'A_Adaptive':    {'w': (0.9, 0.4), 'c1': (2.5, 0.5), 'c2': (0.5, 2.5)},
        'B_HighInertia': {'w': (1.1, 0.4), 'c1': (2.5, 0.5), 'c2': (0.5, 2.5)},
        'C_LowInertia':  {'w': (0.7, 0.2), 'c1': (2.5, 0.5), 'c2': (0.5, 2.5)},
        'D_HighSocial':  {'w': (0.9, 0.4), 'c1': (1.5, 0.5), 'c2': (1.5, 3.0)},
        'E_HighCognitive':{'w': (0.9, 0.4), 'c1': (3.0, 1.0), 'c2': (0.5, 1.5)},
    }
    ...
    # ==================================================================
    # --- 調査したいパラメータセットを選択 ---
    param_sets_to_run = axis2_behavior # ★★★ 調査軸2を実行 ★★★

    # --- 固定するベースラインパラメータ ---
    baseline_cost = axis1_cost['Cost_B_Balanced']
    # baseline_behavior は今回調査対象
    baseline_penalty = axis3_penalty['Pen_B_Standard']

    # --- その他の実験設定 ---
    num_simulation = 30
    node_counts = [1000] # テストするノード数
    ...
    for param_id, current_params in param_sets_to_run.items():
        # ベースライン設定と現在の設定をマージ
        sim_params = {**baseline_cost, **baseline_penalty, **current_params}
        # axis2の調査なので、w, c1, c2はcurrent_paramsのものを必ず使う
        if 'w' in current_params: sim_params['w'] = current_params['w']
        if 'c1' in current_params: sim_params['c1'] = current_params['c1']
        if 'c2' in current_params: sim_params['c2'] = current_params['c2']
    ...
    # --- CSVファイルへの書き出し ---
    file_name = datetime.now().strftime('%Y%m%d_%H%M%S') + '_grid_search_behavior_experiment.csv'
