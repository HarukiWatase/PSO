...
def get_spatial_lbest(swarms, pBests, pBests_fitness, k=5):
    ...
    Args:
        swarms (np.array): 現在の粒子位置 (num_par, dim)
        pBests (np.array): pBest位置 (num_par, dim)
        pBests_fitness (np.array): pBest評価値 (num_par,)
        k (int): 自分を含む近傍粒子の数（デフォルト5）
    Returns:
        lBest_matrix (np.array): 各粒子が目指すべきlBest位置 (num_par, dim)
    """
    ...
    dist_sq = np.maximum(dist_sq, 0) # 演算誤差対策
    
    # 2. 各粒子ごとに距離が近い上位k個のインデックスを取得
    # axis=1 (行ごと) に小さい順にk個選ぶ (自分自身を含む)
...
def simulation(Graph, src, dst, constraints, pso_params):
    ...
    # gBestは「結果記録用」および「収束判定用」として維持する
    ...
    for i in range(num_gen): # 世代ループ
        ...
        for j in range(num_par): # 粒子ループ
            ...
            if is_feasible and bn > gBest_feasible_bn:
                gBest_feasible_bn = bn # 全体最良を更新 (記録用)
        ...
        # --- ★★★ 変更点: 速度更新式 (gBest -> lBest_matrix) ★★★ ---
        # 以前: c2 * r2 * (gBest.reshape(1, -1) - swarms)
        # 今回: c2 * r2 * (lBest_matrix - swarms)
        velocities = w * velocities + c1 * r1 * (pBests - swarms) + c2 * r2 * (lBest_matrix - swarms)
        swarms += velocities

        # (収束判定ロジックは gBest_feasible_bn を使用して継続)
...
if __name__ == '__main__':
    ...
    # ディレクトリ名は適宜変更してください
    result_path = savef.create_dir(dir_name="adaptive_termination_results_Kinbou")
    
    generation_log_filename = result_path + f'/{run_timestamp}_kinbou_generation_log.csv'
    try:
    ...
    summary_filename = result_path + f'/{run_timestamp}_kinbou_summary.csv'
    summary_results = []
    try:
