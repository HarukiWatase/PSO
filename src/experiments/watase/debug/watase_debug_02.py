...
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
# ★★★ 追加: 遅延制約付き修正ダイクストラ法 (デバッグ機能付き) ★★★
# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
def max_load_path_with_delay_constraint(
    G, source, target, max_delay, weight="weight", delay="delay", debug_nodes=None
):
    """
    ...
    while pq:
        ...
        # ★★★ デバッグプリント1: どのノードを探索しているか確認 ★★★
        if u in debug_nodes:
            print(f"[DEBUG] Pop from PQ: u={u}, bottle={current_bottle:.2f}, delay={current_delay:.2f}")
        ...
        if current_bottle < best_known[u][0] or \
           (current_bottle == best_known[u][0] and current_delay > best_known[u][1]):
            # ★★★ デバッグプリント2: 経路がなぜスキップされたか確認 ★★★
            if u in debug_nodes:
                print(f"  └ SKIPPED: Dominated by best_known[{u}] = (bottle={best_known[u][0]:.2f}, delay={best_known[u][1]:.2f})")
            continue
        ...
        for v in G.neighbors(u):
            ...
            # ★★★ デバッグプリント3: 隣接ノードの情報を確認 ★★★
            if u in debug_nodes or v in debug_nodes:
                print(f"  [DEBUG] Edge {u}->{v}: new_bottle={new_bottle:.2f}, new_delay={new_delay:.2f}")

            # 遅延制約を超えてしまう経路は探索しない
            if new_delay > max_delay:
                # ★★★ デバッグプリント4: 遅延制約で棄却されたか確認 ★★★
                if u in debug_nodes or v in debug_nodes:
                    print(f"    └ REJECTED: Delay limit exceeded ({new_delay:.2f} > {max_delay:.2f})")
                continue
            ...
            if new_bottle > best_known[v][0] or \
               (new_bottle == best_known[v][0] and new_delay < best_known[v][1]):
                ...
                # ★★★ デバッグプリント5: 経路が更新されたことを確認 ★★★
                if u in debug_nodes or v in debug_nodes:
                    print(f"    └ UPDATED: Pushing to PQ. best_known[{v}] = ({new_bottle:.2f}, {new_delay:.2f})")
            ...
            # ★★★ デバッグプリント6: 経路が更新されなかった理由を確認 ★★★
            elif u in debug_nodes or v in debug_nodes:
                 print(f"    └ NOT UPDATED: Did not dominate best_known[{v}] = ({best_known[v][0]:.2f}, {best_known[v][1]:.2f})")
...
if __name__ == '__main__':
    ...
    for multiplier in delay_multipliers:
        ...
        for iter_num in range(num_simulation):
            ...
            # 戻り値に src, dst, max_delay を追加しておくと検証が楽になりますが、今回は手動で設定します
            ...
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # ★★★ 追加：逆転現象が発生した場合の検証コード ★★★
            # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
            # PSOが最適解アルゴリズムの結果を上回った場合にのみ実行
            if pso_bn > constrained_mod_dijkstra_bn:
                print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print("!!! 警告: PSOが最適解を上回る結果を報告しました !!!")
                print("!!! この解の有効性を再検証します...")
                
                # simulation関数からsrc, dst, max_delayを返してもらうのが理想ですが、
                # ここではpso_delayとconstrained_mod_dijkstra_bnの結果を使って簡易的に検証します。
                # この検証では、pso_delayが本当にmax_allowable_delay以下だったかを確認するのが主目的です。
                
                print(f"  - PSOの報告ボトルネック: {pso_bn}")
                print(f"  - PSOの報告遅延: {pso_delay}")
                print(f"  - 最適解のボトルネック: {constrained_mod_dijkstra_bn}")

                # simulation内で計算されたmax_allowable_delayは、pso_delayと比較すれば制約違反かどうかわかる
                # （ただし、max_allowable_delayの値そのものを表示するには、simulationから返す必要がある）
                if pso_bn > 0 and constrained_mod_dijkstra_bn == 0:
                     print("  >>> 検証結果: 最適解アルゴリズムは解を見つけられませんでしたが、PSOは有効な解を発見しました。")
                elif pso_delay > 0: # pso_delayが0の場合は無効な経路
                     print("  >>> 検証結果: PSOの解は、報告された遅延を見る限り有効なようです。最適解アルゴリズムの挙動に問題がある可能性があります。")
                else:
                     print("  >>> 検証結果: PSOの解は無効です（遅延計算不能）。")
